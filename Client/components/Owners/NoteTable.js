...
const OwnersTable = (props) => {
    ...
    const ownerRows = owners.map(owner => {
        ...
        let classes = `small ${!!owner.isNew ? 'table-success' : ''}`;
        
        return (
            <tr key={owner.id.toString()} className={classes}>
                <td className="align-middle" style={{width: '80px'}}>
                    <div className="d-flex flex-row">
                        {/* <a data-toggle="tooltip" data-placement="top" title="Edit Owner" className="p-2" onClick={() => props.onOpenEditNoteModal(note.id)}>
